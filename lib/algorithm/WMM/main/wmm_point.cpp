...
int main() {
    ...
    /* Memory allocation */
    // [done] read and store  "WMM.COF";
    
    TimedMagneticModel = MAG_AllocateModelMemory(NumTerms); /* For storing the time modified WMM Model parameters */
    if(MagneticModels == nullptr || TimedMagneticModel == nullptr){MAG_Error(2);}
    TimedMagneticModel = allocate_coefsArr_memory(nMax, MagneticModels); // inside the function numTerms is calculated again instead of using the one in line 64, depending on a condition
