 ...
 */
{
    ...
    int NumberOfTermsPlusOne = NumTerms + 1;
    ...
    MagneticModel->Main_Field_Coeff_G = (double *) malloc((NumberOfTermsPlusOne) * sizeof(double));
    ...
    MagneticModel->Main_Field_Coeff_H = (double *) malloc((NumberOfTermsPlusOne) * sizeof(double));

    if(MagneticModel->Main_Field_Coeff_H == NULL) {
        MAG_Error(2);
        return NULL;
    }
    MagneticModel->Secular_Var_Coeff_G = (double *) malloc((NumberOfTermsPlusOne) * sizeof(double));
    if(MagneticModel->Secular_Var_Coeff_G == NULL) {
        MAG_Error(2);
        return NULL;
    }
    MagneticModel->Secular_Var_Coeff_H = (double *) malloc((NumberOfTermsPlusOne) * sizeof(double));
...
MAGtype_SphericalHarmonicVariables* MAG_AllocateSphVarMemory(int nMax) {
    ...
    int nMaxPlusOne = nMax + 1;
    MAGtype_SphericalHarmonicVariables* SphVariables;
    SphVariables  = (MAGtype_SphericalHarmonicVariables*) calloc(1, sizeof(MAGtype_SphericalHarmonicVariables));
    SphVariables->RelativeRadiusPower = (double *) malloc((nMaxPlusOne) * sizeof ( double));
    SphVariables->cos_mlambda = (double *) malloc((nMaxPlusOne) * sizeof (double));
    SphVariables->sin_mlambda = (double *) malloc((nMaxPlusOne) * sizeof (double));
    return SphVariables;
...
int MAG_SecVarSummation(MAGtype_LegendreFunction *LegendreFunction, 
    ...
    if(fabs(cos_phi) > 1.0e-10) {MagneticResults->By = MagneticResults->By / cos_phi;} 
    else
        ...
        MAGtype_Geoid *Geoid)
{
    ...
    if(MagneticModel->SecularVariationUsed == 1) {
        ...
        if(Geoid->UseGeoid == 1) {printf("Altitude:	%.2f Kilometers above MSL\n", SpaceInput.HeightAboveGeoid);}
        else {printf("Altitude:	%.2f Kilometers above WGS-84 Ellipsoid\n", SpaceInput.HeightAboveEllipsoid);}
        printf("Date:		%.1f\n", TimeInput.DecimalYear);
        printf("\n	Main Field\n");
